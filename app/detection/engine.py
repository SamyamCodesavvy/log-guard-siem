...
def create_alert(
    db: Session, rule_name: str, severity: str, description: str, log: Log
) -> Alert:
    alert = Alert(
...
def _rule_root_login(db: Session, log: Log):
    ...
    if log.action == "root_session" or (
        log.action == "successful_login" and log.username == "root"
    ):
        create_alert(
            db,
            "Root Login Detected",
...
def _rule_credential_sharing(db: Session, log: Log):
    ...
    distinct_users = (
        db.query(func.count(Log.username.distinct()))
        .filter(
            Log.source_ip == log.source_ip,
